...
Because species are interdependant, species loss can have cascading effects and
triger secondary extinctions [@dunne_network_2002]. So assessing the robustness
of ecological networks to species losses has been a big question in Ecology.
Previous research showed that food-web structure, such as connectance, increased
...
metric has some limitations, because each trophic link does not contribute
equally to the robustness of an ecological network. Trophic links can be as
...
not trigger secondary extinctions, but makes the food-webs more fragile to
further extinctions [@allesina_functional_2009].

The identification of functional and redundant links is based on the use of
denomitor trees [@bodini_using_2009; @allesina_who_2004;
...
functional links, and the redundant links. Pioneer papers explained the
algorithm to compute the set of dominator trees and I refer to them for further
information [@bodini_using_2009; @allesina_who_2004; @allesina_functional_2009]
and examples. Here, we aim to show that while implementing this algorithm could
seem somewhat intimitating at the time, this algorithm is now widely
implemented. In R, it is available in `igraph::dominator_tree()`. Below we show
how functional and redundant links can be computed easily from R by replicating
three examples presented in previous studies.
...
the number and the proportion of functional and redundant links.

...
of biodiversity changes[@hillebrand_biodiversity_2018; @blowes_local_no_date]
...
@stouffer_comparmentalization_2011]. Higher connectance, which measures the
...
data[@shanafelt_stability_2018; @danet_species_nodate]. Then, accounting for how
...
biomass and the ones of species richness ($r_\delta = `r rdtps$bm$r`$,
Fig. \@ref(fig:sem) b), meaning that both temporal trends
...
richness ($r_\delta = `r rdsp$ct$bm`$). We found a negative 
relationship between the temporal trends of connectance and the
ones of species richness ($r_\delta = `r rdtps$ct$r`$, Fig. \@ref(fig:sem)d)
...
strength as the temporal one ($r_\delta = `r rdsp$tlvl$bm`$, Fig.
...
($r_\delta = `r rdsp$tlvl$r`$, Fig. \@ref(fig:std)).
...
positive one through community biomass
($r_\delta$ = `r ridtps$ct$r`). It resulted a relatively weak total negative
effect of species richness on connectance (`r rttps$ct$r`). Spatially,
we also found a positive indirect effect of richness on connectance through
community biomass but a weak one (`r ridsp$ct$r`), resulting in a strong negative total
effect of richness on connectance (`r rtsp$ct$r`).

The relationships among temporal trends showed a indirect positive effect of
species richness on average trophic level (`r ridtps$tlvl$r`), resulting in a total positive
effect of species richness on average trophic level (`r rttps$tlvl$r`). Over the spatial
gradient, we found the same positive indirect effect of richness through
community biomass (`r ridsp$tlvl$r`). WHATTTT? But as the direct effect of
richness on average trophic level was strongly negative, it resulted in a total
strong negative effect of species richness on average trophic level over the
spatial gradient (`r rtsp$tlvl$r`).

---
title: "Temporal dynamics of food-web structure in response to biodiversity changes in riverine ecosystems"
author: "Alain Danet (a,b), Loubna El Madouri (a,c), Maud Mouchet (a), Elisa Thébault (d) and Colin Fontaine (a)"
...
# Abstract (250 / 250 words max)

While there are widespread evidence of biodiversity decline, little is known how
food-web disassemble in response. Such knowledge is crucial as food-web
structure determine ecosystem functioning and stability to perturbations.
However, assessing how changes in biodiversity affect changes in food-web
structure require the availability of food-web timeseries and to account for the
differential effects of different biodiversity facets on food-web structure.
Here, we used a standardized monitoring of riverine fish communities over the
last decades (1995-2018) in France, from which we inferred the food-web
structure for each sampling events using diet data and allometric scaling rules.
We estimated the temporal trends in community biomass, species richness, and
food-web structure using hierarchical Bayesian models. We assessed the effects
of community biomass and species richness temporal changes on temporal changes
in food-web structure: connectance, average trophic level and redundancy. We
further assess the generality and robustness of our findings by replicating the
analysis over space and using theoretical simulations with a bioenergetic model.
Our results suggest that food-webs that experienced a decline in community
biomass experienced conjointly a loss of species at upper trophic levels and
conversely. While species richness temporal trends had less clear effects on
food-web structure, mainly due through its indirect effects on community biomass
temporal trends. Both community biomass and species richness declines were
associated with a decrease in redundancy, raising concerns about robustness of
such communities to further perturbations. Our study highlights how biodiversity
changes affect food-web structure, raising concerns about consequences on
ecosystem functioning and robustness.



# Significance (109 / 120 words max)

Biodiversity temporal changes can lead to simultaneous changes in food-web
structure, which is turn can have consequences on ecosystem functioning and
stability. We provide empirical evidence that temporal changes in community
biomass and species richness are linked to changes contrasted assembly and
disassembly processes in food-webs in natural settings. We then show that loss
in both community biomass and species richness are related to a loss of
redundancy in the food-webs, which raise concerns about the robustness of such
communities to perturbations. We further added spatial and theoretical evidence
suggesting that our results are quite robust and open to assess how biodiversity
changes might affect ecosystem functioning and robustness.

# Introduction (487 words)

Biodiversity is experiencing massive declines due to continuously increasing
human pressures, which is expected to have striking consequences on ecosystem
functioning and fragility. It is increasing recognised that biodiversity
changes are widely heterogeneous spatially [@mcgill_fifteen_2015;
@blowes_geography_2019], such as species richness declines and increases can be
related to spatial variations in human pressures [@cardinale_is_2018;
@danet_past_2024]. Biodiversity changes result in altered community structure
[@dornelas_assemblage_2014; @hillebrand_biodiversity_2018; @danet_past_2024],
but so far, one key component of biodiversity, species interactions, has been
mostly ignored by studies assessing temporal changes in biodiversity, despite
the importance of biotic interactions for species coexistence, ecosystem
functioning and stability [@chesson_mechanisms_2000; @thebault_food-web_2003;
@oconnor_general_2017; @danet_species_2021]. A better understanding of
biodiversity changes will then need to account how biodiversity changes alter
the structure of ecological networks.

Yet, both species richness and food-web structure have a significant impact on
coexistence [@chesson_interaction_2008], ecosystem functioning
[@thebault_food-web_2003; @wang_biodiversity_2018], stability
[@danet_species_2021; @eschenbrenner_diversity_2023], and ecosystem robustness
to perturbations [@dunne_network_2002; @allesina_functional_2009;
@bodini_using_2009; @allesina_functional_2009]. Many predator species play a
keystone role by protecting preys from competitive exclusion
[@heithaus_predicting_2008; @johnson_rarity_2007; @estes_trophic_2011], thereby
maintaining species richness. Food-web with higher trophic levels also increase
ecosystem productivity [@thebault_food-web_2003; @wang_biodiversity_2018] and
community temporal stability [@danet_species_2021;
@eschenbrenner_diversity_2023]. Finally, the number and distribution of trophic
links in food-webs determine the robustness of communities to perturbations
[@dunne_network_2002; @allesina_functional_2009; @bodini_using_2009;
@allesina_functional_2009]. Accounting for how food-web assemble and disassemble
according to biodiversity changes is then crucial to evaluate the consequences
of biodiversity changes on ecosystem functioning and stability.

However, assessing the effects of biodiversity changes on food-web structure
requires the availability of multiple temporal timeseries of biodiversity and
food-webs structure, to disentangle the direct and indirect effects of
biodiversity changes on food-web structure, and to account for theories about
the multiple constraints on food-web assembly. We propose to monitor three
structural properties of food-webs and two common monitored biodiversity facets
(i.e. community biomass and species) that are grounded in food-web assembly,
ecosystem functioning, and stability research areas: connectance (i.e. degree of
generalism), weighted average trophic level (i.e. vertical location of biomass),
and redundancy (i.e. the proportion of redundant links). There are pieces of
evidence to connect those food-web structural metrics to changes in ecosystem
functioning and robustness that we review before to present the temporal
timeseries that allows to assess them.

Food-web assembly theory provides predictions on how food-web structure should
change according to changes in community biomass and species richness. The
energetic paradigm and the trophic theory of biogeography
[@lindeman_trophic-dynamic_1942; @gravel_trophic_2011] stand that the presence
of top predators is conditional on the presence of preys in number enough that
it can provide enough energy from the bottom to the top to sustain high trophic
level species. In turn, it can that the presence of generalist predators, which is
positively associated to average trophic level and connectance, will be linked
more to community biomass changes than to species richness. In turn, connectance
and average trophic level are commonly reported to decrease with as species
richness increases [@dunne_network_2006; @danet_species_2021] because predators
are limited in the number of species they can consume and conversely preys are
limited in the number of predator they can sustain. Food-web structure has also
been linked to the robustness to species extinctions [@dunne_network_2002;
@allesina_who_2004; @bodini_using_2009]. Historically having being linked to
connectance, i.e. the number of links, more recent studies have proposed to
separate links that are functional or redundant [@allesina_who_2004;
@allesina_functional_2009; @bodini_using_2009]. The idea is that the loss of
functional links will trigger secondary extinctions because it suppresses
obligatory pathways from resources to consumers while the loss of redundant
links do not remove such obligatory pathways [@allesina_who_2004]. It means that
redundant trophic links enhance ecosystem robustness as they can be safely
removed without triggering secondary extinctions. Thereby we expected that
higher species richness results in higher redundancy in food-webs, meaning that
they create more independent pathways connecting basal producers to higher
trophic level. Finally, biodiversity and ecosystem functioning theory suggest
that in context of significant niche complementarity, community biomass should
be positively related to species richness [@wang_biodiversity_2018;
@schneider_animal_2016], such as it is necessary to account for the indirect
effects of species richness on food-web structure through community biomass.

Here, we report on a study that combine empirical assessment of food-web
structure and biodiversity temporal changes in natural settings, extinction
simulations in empirical food-webs, and theoretical simulations using
bioenergetic model. The availability of food-web timeseries used to be scarce
[@kortsch_disentangling_2021], but the increasing availability of biodiversity
timeseries and the development of methods to reconstruct food-webs
[@bonnaffe_comparison_2021; @gravel_inferring_2013; @danet_species_2021] makes
possible to overcome this limitation. We used a standardized monitoring of fish
riverine communities on the period 1995-2018 over France, documenting the
individual size of fishes in `r nb_st` sites during ten years or more (Table S1). We
reconstructed around `r nb_op` food-webs based on allometric relationships and
food diet data, and then computed connectance, and average trophic level
weighted by trophic species biomass as well as community biomass and species
richness (Fig. \@ref(fig:matmet)). This dataset and method has already been used
in previous studies [@danet_species_2021; @bonnaffe_comparison_2021;
@bonnaffe_interaction_2024]. Using hierarchical Bayesian models, we estimated
temporal trends in food-web structure, community biomass and richness at each
site [@danet_past_2024]. We then used structural equation models (SEM) to
assess the effects of community biomass and species richness temporal changes on
changes in food-web structure, while accounting for the effects of species
richness on community biomass. We then assessed if those effects are holding in
space, i.e. if the space-for-time substitution hypothesis hold
[@blois_space_2013; @damgaard_critique_2019; @adler_contrasting_2007]. We did so
by considering median values of food-web structure and community structure over
the timeseries at each site (Fig. \@ref(fig:matmet)). Finally, we ran
simulations using the bioenergetic model to assess if our results hold beyond
our empirical fish food-webs.

```{r}
fig_mat_cap <- paste0("Using sites of stream fishes in French streams that
  have been monitored at least 10 years, 1) we inferred the food-web at each site
...
CI], Table S3), meaning that both temporal trends had the same direction in average. In
...
both number and proportion of piscivorous species present (Fig. S2a), meaning
...
\@ref(fig:std) and S3). In both empirical data and theoretical simulations, community
...
strong evidence in theoretical simulation ($r_\delta =$ `r rdm$ct$bm`, Fig.
\@ref(fig:std)).
...
associated with the temporal trends of species richness (Fig. S2), meaning that
...
lines, Fig. S4a). At the opposite, removing first low trophic level species first
increase connectance and average trophic level (green lines, Fig. S4a). In turn,
...
```{r}
std_cap <- paste0(
  "Direct and total effects of community biomass and species richness on
  food-web structure. Standardized coefficients ($r_\\delta$) derived from the
  temporal, spatial, and bioenergetic model Structural Equation Models. Points
  and lines display the estimated effects and their confidence intervals at
  95\\%. Values are displayed in Table S3, R-squared in Table S4, and the
  visualisation of the bivariate relationships for the spatial and bioenergetic
  model SEMs are displayed in Figure S3.")

```

```{r std, fig.cap=std_cap, out.width = "100%"}
tar_load(p_semeff_tot_ok_file)
knitr::include_graphics(p_semeff_tot_ok_file)
```
...
food-web structure. Community biomass changes spanned from `r grbmn`% to
+`r grbmx`% per decade, species richness from `r grrhn`% to +`r grrhx`% per
decade, in line previous reports of the heterogeneity of local biodiversity
changes in terrestrial, marine and freshwater realms[@dornelas_assemblage_2014;
...
@li_relationship_2018; but see @grace_integrative_2016]. This result is in
...
species experience a increase in community biomass. This result is coherent the
...
predators [@takimoto_ecosystem_2008; @post_ecosystem_2000]. (Add something about
...
dynamics while spatial gradients reflect aggregated dynamics over a longer
timescale including a far number of processes including high species turnover
along spatial gradients. Our results then suggest community biomass changes in
space might be a good surrogate to infer assembly and disassembly of food-web in
time, but that it is probably not always the case for species richness.
...
@bodini_using_2009], such a higher proportion of redundant interactions
...
@danet_species_2021. The fishes are sampled using

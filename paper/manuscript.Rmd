---
title: "Species richness and biomass temporal trends reveals different assembly processes of food-web in natural fish assemblages " 
...
## Trends of biodiversity needs to be assessed for multiple facets 
...
across small time step [warren_spatial_1969; @pecuchet_species_2020;
@kaartinen_high_2012].
...
In this line, how the temporal changes of community biomasse are related to
...
inference based on allometric relationships and food diet datas, we assessed the
...
patterns than relationships obtained along spatial gradient.
...
beginning of their monitoring tend to display an biomass increase through time
...
richness ($dR$) in order to to test the relationship among the temporal dynamics of network
...
adding $\beta_3dBdR$), the
results are presented in supplementary material (Fig. SXX).
...
res_trends <- class_trends_bm_rich %>%
  ...
  summarise(n = n()) %>%
  group_by(variable) %>%
  mutate(total = sum(n), percent = n / sum(n) * 100 %>% round(., 2))
```
...
`r res_trends[res_trends$variable == "log_bm_std", ]$total` stations displayed monotonic trends for biomass. Among them, 
...
`r res_trends[res_trends$variable == "log_rich_std", ]$total` stations displayed monotonic trends.
...
richness ($r_\delta = `r sp_b_sest["ct"]`$). However, we did not observed
...
Spatially and temporally, species richness scaled positively with
...
linked to increase of community biomass in line with previous studies suggesting
that the presence of higher predators can lead to higher community biomass
[@wang_biodiversity_2018; @danet_species_nodate]. Increase of average trophic
level can be associated to either gain of biomass in the highest trophic level
or losses of biomass in the lowest trophic. In our case, such gain or losses of
average trophic level is probably associated to the variations of the presence
of large fishes. The finding that the evenness of biomass, measured by the
pielou, decreased through time as community biomass increased and reversely,
meaning that biomass distribution across species become more even when community
biomass decreased and reversely that biomass distribution across species become
less even when community biomass increased. In the same line, the temporal
trends of community biomass were positively correlated to the temporal trends of
the proportion of piscivorous species and trophic species in the community (Fig.
SXX). 

We also found in space that evenness of biomass was positively related to
community biomass while evenness of abundances had no relation (Fig. SXX). It
constrasts with previous studies which found a negative relationship between
biomass and evenness of abundances in marine and plant communities
...
theoretical work showing that  higher food-web connectance can result in higher
community biomass [@thebault_food-web_2003]. But it contrasts with previous
results [@poisot_trophic_2013; @danet_species_nodate], highlighting that the
relation between connectance and community biomass may be dependant of assembly
processes at work.

##  Biomass and richness variation are not the same process 

In contrasts to the results for the community biomass, the temporal trends of
evenness of biomass was positively related to the temporal trends of species
richness, meaning that as species richness temporally increase, the
distribution of biomass become more even, while temporal trends of evenness of
abondances had no significant relationship with species richness (Fig. SXX).
Those results seem indicate that biomass trends are linked to few big individuals
while richness trends are linked to overall shift in dominance in communities.  
...
the structure of food-webs might not be very sensitive to variation of species
richness and that species loss and gain concerns species of low trophic level,
which in turn affect connectance very little. Remarkably, temporal
variations of community biomass and the ones of species richness seem to be
linked to high trophic level in the first while being linked to lower trophic
level in the second. Those results highlight that variations in community
biomass and species richness are linked to different assembly processes and/or
pressures, but it is still an open question.
...
the evenness of abundances is positively related to the one of species
...
suggesting that the species loss and gain concern lower trophic levels, which
are also in transmitting energy toward higher level of the trophic chain.

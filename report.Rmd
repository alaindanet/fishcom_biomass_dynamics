...
pred_log_bm_quad2 <- summary_log_bm_f3y %>%
  filter(model == "mod_bm_quad2", variable %in% get_com_str_var()) %>%
  mutate(
...
pred_log_bm_quad2_plot <- map2(
  filter(model_log_bm_f3y, variable %in% get_com_str_var())$data,
  pred_log_bm_quad2$pred_df,
  plot_pred_data)
...
```{r}
pred_log_bm_medium <- summary_log_bm_f3y %>%
  filter(model == "mod_medium_bm", variable %in% get_com_str_var()) %>%
  mutate(
    pred = map(model_obj, ~ggpredict(.x,
	terms = c("bm_slope [-0.2,-0.00001,0.00001,.2]", "inc_f", "bm [quart2]"))),
    pred_df = map(pred, as_tibble),
    plot = map(pred, ~plot(.x, add.data = FALSE)),
    #myplot = map2(pred, , ~plot_final_model(ggpred = .x))
  )
plot_inc <- map2(
  pred_log_bm_medium$pred,
  filter(model_log_bm_f3y, variable %in% get_com_str_var())$data,
  ~plot_final_model(ggpred = .x, rawdata = .y))

plot_inc %<>% map(., ~.x + theme(legend.position = "none"))
medium_plot <- plot_grid(plotlist = plot_inc, labels = pred_log_bm_medium$variable)
```

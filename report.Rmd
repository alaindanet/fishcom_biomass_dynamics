...
```{r}
loadd(log_bm_net_group_median)
```

```{r, fig.height = 12, fig.width = 12}
log_bm_net_group_median %>%
...
```{r}
loadd(model_log_bm, summary_log_bm)
```

```{r, eval = TRUE}
#https://stackoverflow.com/questions/44457243/regression-tables-in-r-markdown-rmarkdown-html-pdf

unique(summary_log_bm$variable)
model_log_bm_table <- summary_log_bm %>%
...
```{r, fig.height = 12, fig.width = 12}
pred_log_bm_quad2 <- summary_log_bm %>%
  filter(model == "mod_bm_quad2") %>%
  mutate(
...
```{r}
loadd(bm_net_group_median, log_bm_net_group_median, bm_vs_net_trends)
bm_vs_net_trends %>%
...
```{r}
pred_log_bm_quad <- summary_log_bm %>%
...
```{r}
loadd(log_bm_net_group_median, vif_log_bm)
...
```{r}
loadd(model_log_bm_by_protocol, summary_log_bm_by_protocol)
```

```{r, eval = FALSE}
complete_log_quad2 <- summary_log_bm_by_protocol %>%
...
```{r, eval = FALSE}
filter(summary_log_bm_by_protocol, model == "mod_log_bm_inc")[["resume"]]
filter(summary_log_bm_by_protocol, model == "mod_log_bm_inc")[["anova"]]

filter(summary_log_bm_by_protocol, model == "mod_bm_quad")[["resume"]]
filter(summary_log_bm_by_protocol, model == "mod_log_bm_inc")[["anova"]]
```
...
```{r}
pred_log_bm_quad2 <- summary_log_bm_by_protocol %>%
  filter(model == "mod_bm_quad2", protocol_type == "complete") %>%
  mutate(
    pred = map(model_obj, ~ggpredict(.x,
	terms = c("bm_slope", "bm [quart2]"))),
...
```{r}
pred_log_bm_quad <- summary_log_bm_by_protocol %>%
...
```{r, eval = FALSE}
pred <- summary_log_bm_by_protocol %>%

...
model_log_bm_f3y_table <- summary_log_bm_f3y %>%
...
model_log_bm_f3y_table[["reg_table"]] %>% 
  bind_rows(.) %>%
  mutate(variable = rep(model_log_bm_f3y_table[["variable"]], each = 6)) %>%
...
```{r}
bind_rows(model_log_bm_f3y_table[["anova_table"]]) %>% 
  mutate(variable = rep(model_log_bm_f3y_table[["variable"]], each = 6)) %>%
  select(variable, everything()) %>%
  kable(., booktabs = TRUE,
    caption = 
...
```{r}
map_dfr(model_log_bm_f3y_table[["model_obj"]], broom::glance) %>%
  mutate(variable = model_log_bm_f3y_table[["variable"]]) %>%
  select(variable, everything()) %>%
  kable(., booktabs = TRUE,
    caption = 
      paste0(
...
```{r, eval = FALSE}
complete_log_quad2 <- summary_log_bm_f3y_by_protocol %>%

...
- Species richness:
  ...
  - increases slightly in time when biomass increases (immigration debt)
...
pred_plot_signif_com <- predict_plot %>%
  filter(
    y %in% c(get_com_str_var(), "log_rich_std"),
    x == "log_bm_std"
  )
...
top <- plot_grid(
  ...
  labels = pred_plot_signif_com$y, nrow = 1
)
...
pred_plot_rich <- predict_plot %>%
  filter( y %in% c(get_com_str_var(), "bm_std", "log_bm_std"), x == "log_rich_std") %>%
...
pred_plot_other <- predict_plot %>%
  filter( y %in% c("ct_ff", "w_trph_lvl_avg"), x == c("ct_ff", "w_trph_lvl_avg"))
legend_signif <- get_legend(
...
```{r, fig.dim = c(12, 8)}
plot_grid(
  pred_plot1, plot_grid(pred_plot2, pred_plot3, ncol = 2),
  nrow = 2
...
model_log_bm_f3y_table_medium[["anova_table"]] %>%
  filter( y %in% c(get_com_str_var(), "ct_ff", "log_rich_std")) %>%
  kable(., booktabs = TRUE,
    caption = 
      paste0(

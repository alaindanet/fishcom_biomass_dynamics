...
model_bm_vif %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_grid(model_type~variable, scales = "free_x")
```
...
```{r}
filter(model_log_bm_summary, model == "mod_inc_bm")[["resume"]]
filter(model_log_bm_summary, model == "mod_inc_bm")[["anova"]]

filter(model_log_bm_summary, model == "mod_bm_inc")[["resume"]]
filter(model_log_bm_summary, model == "mod_bm_inc")[["anova"]]

filter(model_log_bm_summary, model == "mod_bm_quad")[["resume"]]
filter(model_log_bm_summary, model == "mod_bm_quad")[["resume"]] %>%
  map(., ~ .x$r.squared)
filter(model_log_bm_summary, model == "mod_bm_quad")[["anova"]]


filter(model_log_bm_summary, model == "mod_bm_quad2")[["resume"]]
filter(model_log_bm_summary, model == "mod_bm_quad2")[["resume"]] %>%
  map_dbl(., ~ .x$r.squared)

filter(model_log_bm_summary, model == "mod_bm_quad2")[["anova"]]

filter(model_log_bm_summary, model == "mod_log_bm_quad2")[["anova"]]
filter(model_log_bm_summary, model == "mod_log_bm_quad2")[["resume"]] %>%
  map_dbl(., ~ .x$r.squared)
```
...
```{r}
filter(model_log_bm_by_protocol_summary, model == "mod_inc_log_bm")[["resume"]]
filter(model_log_bm_by_protocol_summary, model == "mod_inc_log_bm")[["anova"]]
```

...
- There was a significant positive link between the dynamics of community biomass and the
  dynamics of community structure for the variables: species richness, average
  trophic level, connectance except for pielou of individuals and pielou of
  biomass which had respectively a no and a negative relationship. 

- There was a significant positive link between the dynamics of species richness
  and the dynamics of community structure for the variables: average trophic
  level, connectance, pielou of individuals and pielou of biomass; except for
  connectance which had a negative relationship.
...
- We found that the link between network temporal dynamics and biomass temporal
  dynamics **does not change** according if the community loses or gain biomass,
  for all variables.

## Answer to research questions 

- Lost and gain of biomass or species richness resulted in same network
  structure dynamics, suggesting that the two processes are symmetric.
...
loadd(p_fig1_2)
loadd(predict_plot2)
pl <- get_plot_fig1_2(predict_plot2, get_list = TRUE)

map(pl$bm, function (plo) {
  plo + theme(legend.position = "none")
  
  save_plot(
    file = get_mypath(
      "figures", 
      paste0(str_replace_all(plo$labels$y, " ", "_"),
	"_",
	str_replace_all(plo$labels$x, " ", "_"),
	".png"
      )
      ),
    plot = plo,
    base_height = 4, base_asp = 1.2#1.618 
  )

} 
)
map(pl$rich, function (plo) {
...
pred_plot1 <- p_fig1_2[["bm"]]
pred_plot2 <- p_fig1_2[["rich"]]
pred_plot3 <- p_fig1_2[["other"]]
pred_plot4 <- p_fig1_2[["nind"]]
```
...
summary_var_f3y %>%
  na.omit() %>%
  select(bm_std, nind_std, log_bm_std, connectance, ct_ff, rich_std, w_trph_lvl_avg,
...
legend_signif2 <- get_legend(
  pred_plot_signif$pred_plot[[1]] + theme(legend.position = "bottom")
)
...
```{r pred-other-com-rich, fig.dim = c(12, 10)}
pred_plot_signif <- predict_plot2 %>%
  filter(!y %in% c(get_com_str_var(), "ct_ff"), x == "log_rich_std")
legend_signif2 <- get_legend(
  pred_plot_signif$pred_plot[[1]] + theme(legend.position = "bottom")
)
pred_plot_signif$pred_plot %<>% map(., ~.x + theme(legend.position = "none"))

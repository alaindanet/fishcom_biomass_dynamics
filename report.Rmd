...
median_biomass <- 
  ...
  ungroup()
three_first_year_biomass <-
  full_data2 %>%
  filter(nb_year <= 2) %>%
  select(station, nb_year, biomass, rel_bm, log_bm, rel_log_bm) %>%
  pivot_longer(cols = biomass:rel_log_bm, names_to = "coeff", values_to = "first_year") %>%
  group_by(station, coeff) %>%
  summarise(first_3_year = median(first_year, na.rm = TRUE)) %>%
  group_by(coeff) %>%
  mutate(com_size = ifelse(first_3_year < median(first_3_year, na.rm = TRUE), "little", "big")) %>%
  ungroup()
...
myload(habitat_press, habitat_pressure, op_analysis, envir = environment(),
  dir = get_mypath("data"))
width_classif_mean <- habitat_press %>%
    mutate(width_classif_mean  = if_else(width_river_mean < mean(width_river_mean, na.rm = TRUE), "little",
      "big")) %>%
    select(station, width_river_mean, width_classif_mean)
habitat_press %>%
...
plot_final_model <- function (ggpred = NULL, rawdata = NULL) {
  ggpred %<>%
    as_tibble %>%
    rename(increasing = group, bm_group = facet)
  ...
  p <- ggplot(ggpred, aes(x = x , y = predicted, color = bm_group)) +
    geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = bm_group), alpha =
      .1) +
    geom_line()
